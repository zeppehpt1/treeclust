...
def chooseBestKforKMeansParallel(scaled_data:NDArray, k_range:int) -> Tuple[int, DataFrame]:
    '''
    Parameters 
    ----------
    scaled_data: matrix 
...
def scaled_insertia(scaled_data:NDArray, k_range=range(2, 20)) -> Tuple[int, DataFrame]:
    ans = []
    for k in k_range:
...
def mean_shift(reduced_f:NDArray, y_gt:list, RANDOM_SEED:int) -> Tuple[list, float, float, float, float, float, float, float, int]:
    ...
    micro_f1_score, macro_f1_score, weighted_f1_score, nmi, f_star, cohen_kappa, mcc = get_accuracy_value(y_gt, y_pred)
...
def k_means(reduced_f:NDArray, y_gt:list, RANDOM_SEED:int) -> Tuple[list, float, float, float, float, float, float, float, int]:
...
def agglo_cl(reduced_f:NDArray, y_gt:list, RANDOM_SEED:int) -> Tuple[list, float, float, float, float, float, float, float, int]:
...
def fuzzy_c_means(reduced_f:NDArray, y_gt:list, RANDOM_SEED:int) -> Tuple[list, float, float, float, float, float, float, float, int]:
...
def optics(reduced_f:NDArray, y_gt, RANDOM_SEED:int) -> Tuple[list, float, float, float, float, float, float, float, int]:
...
def get_cluster_res(reduced_f:NDArray, y_gt:list, RANDOM_SEED:int) -> Tuple[list, list, list, list, list, list, list, list, list, list]:
    ...
    for fn, ident in zip(fns, fns_idents):
        y_pred, micro_f1_score, macro_f1_score, weighted_f1_score, f_star, cohen_kappa, mcc, nmi, species = fn(reduced_f, y_gt, RANDOM_SEED)
    ...
    return cluster_techniques, micro_f1_scores, macro_f1_scores, weighted_f1_scores, f_star_values, cohen_kappa_scores, mcc_scores, nmi_scores, y_pred_labels, species_pred
...
def get_accuracy_value(y_gt:list, y_pred:list) -> Tuple[float, float, float, float, float, float, float]:
    return f1_score(y_gt, y_pred, average='micro'), f1_score(y_gt, y_pred, average='macro'), f1_score(y_gt, y_pred, average='weighted'), v_measure_score(y_gt, y_pred), f_star(y_gt, y_pred), cohen_kappa_score(y_gt, y_pred), matthews_corrcoef(y_gt, y_pred)
