...
def normalize_img(img_arr: NDArray) -> NDArray:
    normalized_img = cv2.normalize(
        img_arr, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F
    )
...
def preprocess_images(
    ...
    # create dir
    out_dir = Path(images_path).parent / Path(
        "preprocessed_"
        + str(expected_size)
        + enhancements
        + "_clipped_pred_"
    ...
    if os.path.isdir(out_dir) == False:
        ...
        for img in (pbar := tqdm.tqdm(file_list)):
            ...
            # resize
            if resize == "padding":
                PIL_image = resize_with_padding(
                    PIL_image, (expected_size, expected_size)
                )
            elif resize == "stretch":
                PIL_image = PIL_image.resize((expected_size, expected_size))
            # save img
            PIL_image.save(
                str(out_dir) + "/" + str(file_name) + "_preprocessed" + ".png"
            )
        return out_dir
    else:
