...
def get_basic_df(csv_dir: Union[str, PosixPath]) -> DataFrame:
    ...
    new_df = df.get(
        ["Preprocess", "CNN", "DR", "Clustering"]
    )  # or include pred labels with 'Pred labels'
    return new_df
...
def get_micro_f1_mean(
    csv_dir: Union[str, PosixPath]
) -> Tuple[list, str]:  # is essentially 'accuracy' in a multi-class scenario
...
def calc_multiple_means(
    encoding_path: Union[str, PosixPath],
    le_path: Union[str, PosixPath],
    RANDOM_SEEDS: list,
    n_interval: int,
    dr: str,
    num_clusters: int,
    ...
    for SEED in (pbar := tqdm(range(len(RANDOM_SEEDS)))):
        ...
        if dr == "umap":
            reducer = umap.UMAP(
                n_components=2, metric="cosine", random_state=RANDOM_SEEDS[SEED]
            )
...
def calculate_mean_iou_for_multiple_polygons(
    ground_truth_df: GeoDataFrame, prediction_df: GeoDataFrame
) -> Tuple[float, int]:
    iou_scores = []
    errors = 0
...
def calculate_object_detection_f1_score(
    ground_truth_df: GeoDataFrame, prediction_df: GeoDataFrame
) -> float:
    ...
    _, count_iou_scores = calculate_mean_iou_for_multiple_polygons(
        ground_truth_df, prediction_df
    )
