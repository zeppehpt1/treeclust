...
def main():
    ...
    if not Path(site_folder + "pred_polygon_clipped_raster_files").exists():
        Path(site_folder + "pred_polygon_clipped_raster_files").mkdir(
            parents=True, exist_ok=True
        )
        prepare.clip_crown_sets_with_gt_masks(
            orthophoto_dir,
            prediction_dir,
            gt_mask_dir,
            make_squares=False,
    ...
    first_set = preprocessing.preprocess_images(
        site_folder + "pred_polygon_clipped_raster_files",
        None,
        None,
        False,
        True,
    ...
    second_set = preprocessing.preprocess_images(
        site_folder + "pred_polygon_clipped_raster_files",
        None,
        None,
        False,
    ...
    if len(encoding_files) != len(cnns) * 2:
        for cnn in cnns:
            clahe_denoising = ft.create_and_save_le_encodings(
                cnn, first_set, site_folder
            )
    ...
    if os.path.exists(result_dir / final_res_name) == False:
        for RANDOM_SEED in (pbar := tqdm(RANDOM_SEEDS)):
            ...
            for encoding_file, le_file in zip(encoding_files, le_files):
                files, features, labels, y_gt = cluster.load_all_data(
                    encoding_file, le_file
                )
                # 4. DR
                ...
                for dr_features, dr_ident in zip(reduced_features, dr_idents):
                    ...
                    (
                        used_cluster_alg,
                        aquired_micro_f1_scores,
                        aquired_macro_f1_scores,
                        aquired_weighted_f1_scores,
                        aquired_f_star_scores,
                        aquired_cohen_kappa_scores,
                        aquired_mcc_scores,
                        aquired_nmi_scores,
                        aquired_y_pred_label_sets,
                        aquired_num_pred_species,
                    ) = cluster.get_cluster_res(dr_features, y_gt, RANDOM_SEED)
            ...
            # save results as pd
            results = {
                "Preprocess": preprocessing_types,
                "CNN": cnns,
                "DR": dr_techniques,
